...
####题目处理
def get_title(doc , tit_index):
    title = ''
    title_ele = doc.paragraphs[tit_index]._element.xml
    title_tree =etree.fromstring(title_ele)

    children =title_tree.getchildren()
    for child in children:
        tag = get_tag(child)
        if tag == 'r':
            text = child.findall('w:t')
            print(text , '310')
            exit()
...
####选择题处理
def handle_choice(doc , list):
    for item in list:
        tit_index = item['title'][0]
        ops_index = item['options'][0]
        title = get_title(doc , tit_index)
        option = get_option(doc , ops_index)
...
while (index <12):
    choice_que.append(all_ti_index[0][index])
    index += 1

index = 0
while(index<4):
    completion_que.append(all_ti_index[1][index])
    index += 1

index = 0
while(index<7):
    answer_que.append(all_ti_index[2][index])
    index += 1

handle_choice(doc , choice_que)