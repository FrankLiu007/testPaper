from docx_utils.parse_paper import   parse_answer
...
from docx_utils.parse_all import parse_all
'''
###获取答案
    ...
    for dati in all_ti_index:
        for ti_index in dati:
            ...
            for i in range(0, len(questions)):
                ...
                if 'score' in ti_index['questions'][i]:
...
def docx_paper2json(pars):
    ...
    if pars['subject']=='英语':
        settings.dati_mode=['^第','([一二三四五六七八九])','部分']
        settings.jiemode =['^第','([一二三四五六七八九])','[小]{0,1}节']
    else:
        settings.dati_mode = ['^','([一二三四五六七八九])','[\s\.．、]']
    ...
    all_ti_index=parse_all(doc,0,end_row)
    ###处理试卷
    print('开始处理试卷...')
...
'''
    ...
    print(zz, file=outf)
    for ti in tis:
        print('<div style="border:1px dashed #00F">', file=outf)
